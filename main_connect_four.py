...
# game = ConnectFour(ConnectFourAgent, ConnectFourAgent, 0.8, 0.8)
...
class GameView(object):
    ...
    def initialize_game_variables(self, game_mode, count):
        """
        ...
        if game_mode == "single":
            ...
            if (self.trainedComputer == None):
                print("not trained")
                self.p2 = ConnectFourAgent(second_coin_type)
            ...
            self.win_list = [0,0]
            if(count == 1):
                self.p1 = ConnectFourAgent(first_coin_type)
                self.p2 = ConnectFourAgent(second_coin_type)
    ...
    def run(self, game_mode, iterations=1):
        ...
        while (iterations > 0):
            count += 1
            self.initialize_game_variables(game_mode, count)
            ...
            while not game_over:
                ...
                if not human_turn:
                    learn = True if (game_mode == "train") else False
                    # print("Before game ov")
                    game_over = current_player.complete_move(winner_value, coin,  self.game_board, self.game_logic, self.background,iterations,learn)
                    # print("After game ov")
                    coin_inserted = True
                    uninitialized = True
                ...
                if game_over:
                    ...
                    if (winner_value > 0 and game_mode == "train"):
                        ...
                        current_player.complete_move(winner_value, coin,  self.game_board, self.game_logic, self.background,iterations, learn)
                        winner_value = None
