...
class Model:
    ...
    def calc_target(self, prev_state, prev_move, state, ava_moves, reward):
        ...
        if reward == 0:
            if len(v) > 0:
                v_s_coin_type = self.gamma * np.max(v)
            else:
                print('no moves!!!')
                v_s_coin_type = 0
            target = np.array(v_s + self.alpha * (reward + v_s_coin_type - v_s))
        else:
            # v_s_coin_type = 0
            target = reward
    ...
    def learn_batch(self, memory):
        print('start learning player', self.coin_type)
        ...
        # print('player:', self.coin_type, loss, 'loops', count)
